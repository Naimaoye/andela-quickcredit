...
import userss from '../server/models/userss';
...
describe('/GET user', () =>{
   it('it should get all existing users', (done) => {
   		chai.request(server).get('/api/v1/users').send(userss).end((err, res) => {
...
describe('/POST user', () => {
    ...
    it('it should not create a user if firstname is not present', (done) => {
   		chai.request(server).post('/api/v1/auth/signup').send(userErr).end((err, res) => {
   			res.body.error.should.be.eql('unknown error');
...
describe('Test for Admin mark user as verified', () => {
  describe('PATCH /api/v1/users/:email/verify', () => {
    it('should verify a user', (done) => {
      const Admin = {
        email: 'Naimat@mail.com',
        password: 'password',
      };

      const email = 'doyin@mail.com';
      chai
        .request(server)
        .post('/api/v1/auth/signin')
        .send(Admin)
